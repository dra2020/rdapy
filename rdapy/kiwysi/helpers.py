...
def score_shapes(sample_shapes, predictions, geodesic=True):
    ...
    for i in range(len(sample_shapes)):
        ...
        print("Sample {:2} - Prediction = {:.2f}, Answer = {:.2f}, Delta % = {:>+6.1%}"
              .format(i + 1, calc, correct, ((calc - correct) / correct)))
    print()
    print()
...
def featureize_shapes(sample_shapes, featureized_shapes, geodesic=True):
    ...
    print()
    print("{:<2} = {:<12} | {:<12} | {:<12} | {:<12} | {:<12} | {:<12} | {:<12}"
          .format('##', 'sym_x', 'sym_y', 'reock', 'bbox', 'polsby',
                  'hull', 'schwartzberg'))
    print('-' * 119)
    ...
    for i in range(len(sample_shapes)):
        ...
        deltas = [(calcs[i] - corrects[i]) / corrects[i]
                  for i in range(len(calcs))]

        print("{:2} = {:.10f} | {:.10f} | {:.10f} | {:.10f} | {:.10f} | {:.10f} | {:.10f}"
              .format(sample_ix, calcs[0], calcs[1],
                      calcs[2], calcs[3], calcs[4],
                      calcs[5], calcs[6]), "<<< Calc\'d")
        print("     {:.10f} | {:.10f} | {:.10f} | {:.10f} | {:.10f} | {:.10f} | {:.10f}"
              .format(corrects[0], corrects[1],
                      corrects[2], corrects[3], corrects[4],
                      corrects[5], corrects[6]), "    Correct")
        print("     {:<+10.1%}   | {:<+10.1%}   | {:<+10.1%}   | {:<+10.1%}   | {:<+10.1%}   | {:<+10.1%}   | {:<+10.1%}   "
              .format(deltas[0], deltas[1],
                      deltas[2], deltas[3], deltas[4],
                      deltas[5], deltas[6]), "   Delta %")
        print('-' * 119)
    print()
    print()
