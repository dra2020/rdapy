...
A set of scripts in the `scripts` directoy enable you to "score" large collections of plans in bulk.
This is useful for scoring ensembles of plans, e.g., generated by ReCom.

## SCORE.sh

This bash script takes an ensemble of plans and input data and 
...
*   The `plans` is a JSONL file that contains the ensemble of plans to be scored.
    An example is provided in `testdata/ensemble/NC_congress_plans.100.jsonl`.
...
The example noted above is in that format.
